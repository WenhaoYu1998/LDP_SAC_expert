...
def parse_args():
    ...
    parser.add_argument('--traj_len_arr_len', default=1000, type=int)
    ...
    for step in range(args.num_train_steps):
        ...
        if global_done:
            ...
            for i in range(cfg['agent_num_per_env']):
                ...
                print('delta_w_rate_robot_' + str(i) + ': {}'.format(delta_w_rate))
                
                L.log('eval/arrive_rate_robot_' + str(i), arrive_rate, step)
                L.log('eval/delta_w_rate_robot_' + str(i), delta_w_rate, step)
               

            episode += 1
            L.log('eval/episode', episode, step)
            if episode > 5000:  #5000
                exit()
